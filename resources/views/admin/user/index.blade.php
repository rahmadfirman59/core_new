    ...
    <div class="content flex-column-fluid" id="kt_content">
        <div class="toolbar d-flex flex-stack flex-wrap mb-5 mb-lg-7" id="kt_toolbar">
            ...
            <div class="d-flex align-items-center py-1 gap-6">
                <button type="button" onclick="info()" class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-40px">Tambah User</button>
            </div>
        </div>
                    ...
                    <form id="form_search">
                        ...
                        <div class="card-title d-flex flex-lg-row align-items-center flex-column gap-6">
                            <div class="d-flex align-items-center position-relative my-1 gap-6">
                                ...
                                <x-input name="nama" prefix="search_" caption="Search User" class="form-control-solid w-250px ps-12" />
                            </div>
                            <x-select name="akses" prefix="search_" caption="All Role" :options="$list_akses" class="form-select-solid w-250px" data-control="select2" onchange="search_data()" />
                        </div>
...
@include('components.search_location')
...
@push('scripts')
    <script>
        let $form_search = $('#form_search'),
            ...
            $card_data = $('#card_data'),
            $card_info = $('#card_info');
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ url('admin/user') }}';
