    ...
    <div class="card">
        <form id="form_info">
            ...
            <div class="card-header">
                <div class="card-title fs-3 fw-bold">{{ !empty($program) ? 'Ubah' : 'Tambah' }} Program Kerja</div>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-12">
                        <x-metronic-textarea name="konten"  caption="Pengurus" :value="$program->konten ?? ''" />
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        let $form_search = $('#form_search'),
            ...
            $card_data = $('#card_data'),
            $card_info = $('#card_info');
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ url('admin/profil/program-kerja') }}';
