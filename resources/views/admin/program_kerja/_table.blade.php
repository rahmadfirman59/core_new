<div class="table-responsive">
    <table class="table align-middle table-row-dashed fs-6 table-sm">
        ...
        @php($no = 1)
        @if($program instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($program->currentPage()-1) * $program->perPage()) + 1)
        @endif
        @foreach($program as $s)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
                <td>{{ $s->konten }}</td>
...
<div class="d-flex flex-row justify-content-center">
    @if($program instanceof \Illuminate\Pagination\LengthAwarePaginator)
        {{ $program->links('vendor.pagination.custom') }}
    @endif
</div>
